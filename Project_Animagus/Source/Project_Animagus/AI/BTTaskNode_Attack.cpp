...
bool UBTTaskNode_Attack::CheckShockwaveSkill(UBehaviorTreeComponent& OwnerComp, ABaseCharacter* Character)
{
    ...
    // 거리 조건: 400 이하일 때만 true
    return Distance <= 400.f;
}

#if 0
    // 공격 스킬 목록
    TArray<FString> AttackSkills = {
        TEXT("Fireball"),
        TEXT("MagicMissile"),
        TEXT("RadialSkill"),
        TEXT("ShockwaveSkill"),
    ...
    // 순서대로 사용 시도
    for (const FString& SkillName : AttackSkills)
    {
        if (Character->UseSkillByName(SkillName))
        {
            UE_LOG(LogTemp, Log, TEXT("AttackTask: Successfully used skill: %s"), *SkillName);
            return EBTNodeResult::Succeeded; // 성공하면 바로 종료
        }
    }
